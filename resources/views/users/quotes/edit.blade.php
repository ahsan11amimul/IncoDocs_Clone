...
<body>
    ...
    <div class="row">
     <div class="col-md-12 card mt-2 p-3 bg-light mb-3">
        <div class="card-body"> 
          <h5 class="card-title text-center">EDIT QUOTATION</h5>
		  <form action="{{ route('quotes.update', $quote->id) }}" method="POST" enctype="multipart/form-data">
			@csrf
		     <div class="row">
					<div class="col-6 border">
						<div class="form-row">
							<div class="form-group col-12">
                                ...
                                <option value="{{$quote->seller_id??''}}">{{$quote->seller_id?App\Contact::where('id',$quote->seller_id)->first()->value('first_name'):'Select'}}</option>
							...
							<div class="form-group col-12">
                                ...
                                <option value="{{$quote->buyer_id??''}}">{{$quote->buyer_id?App\Contact::where('id',$quote->buyer_id)->first()->value('first_name'):'Select'}}</option>
									@foreach (App\Contact::where('user_id',Auth::user()->id)->get() as $item)
								<option value="{{$item->id}}">{{$item->first_name}}</option>
									@endforeach
								</select>
					...
					<div class="col-6 border">
						<div class="form-row">
							...
							<div class="col-6 form-group">
								...
								<input type="date" name="date" id="date" class="form-control" value="{{$quote->date??date('Y-m-d')}}" />
							</div>
							<div class="col-6 form-group">
							...
							<div class="col-6 form-group">
								...
								<input
									...
									name="delivery_date"
									id="delivery_date"
                                    value="{{$quote->delivery_date??date('Y-m-d')}}"
						...
						<div class="form-row">
							<div class="form-group col-6">
								...
								<select
									...
									class="form-control"
								>
                            <option value="{{$quote->dispatch_id??''}}">{{$quote->dispatch_id?App\Dispatch::where('id',$quote->dispatch_id)->first()->value('name'):'-'}}</option>
                            ...
                            <input type="text" name="shipment_type" id="shipment_type" class="form-control" value="{{$quote->shipment_type}}">
							</div>
						</div>
						<div class="form-row">
							<div class="form-group col-6">
								...
								<select
									...
									class="form-control"
								>
									<option value="{{$quote->loading_id??''}}">{{$quote->loading_id?App\Place::where('id',$quote->loading_id)->first()->value('name'):'-'}}</option>
							...
							<div class="form-group col-6">
								...
								<select
									...
									class="form-control"
								>
									<option value="{{$quote->discharge_id??''}}">{{$quote->discharge_id?App\Place::where('id',$quote->discharge_id)->first()->value('name'):'-'}}</option>
									@foreach (App\Place::all() as $item)
								   <option value="{{$item->id}}">{{$item->name}}</option>
									@endforeach
								</select>
						...
						<div class="form-row">
							<div class="form-group col-md-12">
								...
								<select
									...
									class="form-control"
								>
									<option value="{{$quote->payment_method??''}}">{{$quote->payment_method?App\Detail::where('id',$quote->payment_method)->first()->value('name'):'Detail'}}</option>
						...
						<div class="form-row">
							<div class="form-group col-md-12">
								...
								<select
									...
									class="form-control"
								>
									<option value="{{$quote->additional_info??''}}">{{$quote->additional_info?App\Detail::where('id',$quote->additional_info)->first()->value('name'):'Details'}}</option>
									@foreach (App\Detail::where('user_id',Auth::user()->id)->get() as $item)
								   <option value="{{$item->id}}">{{$item->name}}</option>
									@endforeach
								</select>
						...
						<div class="form-row">
							<div class="form-group col-md-12">
								...
								<select
									...
									class="form-control"
								>
                              <option value="{{$quote->bank_detail??''}}">{{$quote->bank_detail?App\Detail::where('id',$quote->bank_detail)->first()->value('name'):'Details'}}</option>
									@foreach (App\Detail::where('user_id',Auth::user()->id)->get() as $item)
								   <option value="{{$item->id}}">{{$item->name}}</option>
									@endforeach
								</select>
						...
						<div class="form-row">
							...
							<div class="form-group col-md-4">
								<input
									...
									name="action_date"
									id="action_date"
									value="{{$quote->action_date??date('Y-m-d')}}"
									class="form-control"
							...
							<div class="col-md-12 form-group">
                                ...
                                />
                               <input type="hidden" class="form-control" id="old_image" name="old_image" value="{{$quote->signature}}">
			    ...
			    <input type="hidden" name="hidden_id" value="{{$quote->invoice_number}}">
...
<script>
	$(document).ready(function(){
        ...
        var count = {{ App\OrderItem::where('invoice_number',$quote->quote_number)->get()->count()}};
		//alert('init'+count);
